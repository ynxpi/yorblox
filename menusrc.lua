...
local function toggleFreeze(enabled)
    if enabled then
        ContextActionService:BindActionAtPriority("YnxFreeze", function() 
    ...
    for i, item in ipairs(list) do
        ...
        if type(item) == "table" then
            ...
            elseif item.Type == "Slider" then 
                local val = (self.TypingSlider and self.Index == i) and "" or tostring(item.Value)
                txt = txt .. " <" .. val .. ">"
            end
        end
...
function YorbloxLib:BindControls()
    UserInputService.InputBegan:Connect(function(io, p)
        ...
        elseif io.KeyCode == Enum.KeyCode.Left and self.CurrentPath ~= "Main" then
            self.CurrentPath = "Main"
            self.Index = 1
        elseif io.KeyCode == Enum.KeyCode.Return then -- Fixed KeyCode
            ...
            elseif item.Type == "Slider" then
                ...
                local connection
                connection = self.InputReceiver.FocusLost:Connect(function(enter)
                    if enter then
