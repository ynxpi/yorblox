...
function YorbloxLib.Init(modifier, openKey)
    ...
    self.Screen.ResetOnSpawn = false -- FIX: Keeps menu alive after death
    ...
    self.Bg.BackgroundColor3 = Color3.fromRGB(0, 0, 0) -- FIX: Solid Black
    ...
    glitch.BackgroundTransparency = 0.98 -- Fainter red glitch
...
function YorbloxLib:BindControls()
    UserInputService.InputBegan:Connect(function(io, p)
        -- Combo: Hold RMB + V to Toggle Menu
        ...
        -- FIX: Navigation works without holding RMB
        ...
        elseif io.KeyCode == Enum.KeyCode.A and self.CurrentPath ~= "Main" then
            -- FIX: Explicit Back button (A) so you don't scroll out of categories
            self.CurrentPath = "Main"; self.Index = 1
        end
        self:Refresh()
    end)

    -- Slider Logic (Hold D to increase, A to decrease)
        ...
        if self.Open and self.CurrentPath ~= "Main" then
            ...
            if item and item.Type == "Slider" then
                ...
                elseif UserInputService:IsKeyDown(Enum.KeyCode.A) then
                    -- Slider adjustment uses A, but if you let go it won't trigger "Back"
